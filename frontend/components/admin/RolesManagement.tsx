...
export default function RolesManagement() {
  ...
  const loadRoles = async () => {
    try {
      ...
      setError(err.response?.data?.error || 'Erreur lors du chargement des rôles');
    } finally {
      setLoading(false);
    }
  };
  ...
  const createRole = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/roles', formData, {
        withCredentials: true
      });
      closeModal();
      loadRoles();
  ...
  const updateRole = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:3001/api/roles/${editingRole.id}`, formData, {
        withCredentials: true
      });
      closeModal();
      loadRoles();
  ...
  // Obtenir le nombre d'utilisateurs pour un rôle
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Gérez les rôles et permissions des utilisateurs</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <div key={role.id} className="bg-white rounded-lg shadow-sm border p-6 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {getUserCount(role)} utilisateur(s)
                  </p>
                </div>
              </div>
            ...
            <div className="border-t pt-4">
              <div className="flex items-center justify-between text-xs text-gray-500">
                ...
                <span>Modifié le {new Date(role.updatedAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {roles.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">Commencez par créer votre premier rôle.</p>
          <div className="mt-6">
            <button
              onClick={() => openModal()}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={editingRole ? updateRole : createRole}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Liste des permissions (séparées par des virgules)..."
                    />
                  </div>
                </div>
